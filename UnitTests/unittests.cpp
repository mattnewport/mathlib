...
TEST_CLASS(MatrixUnitTests){public : TEST_METHOD(TestMatrixTypeTraits){
...
TEST_METHOD(TestMatrixBasics) {
    ...
    m7 += m0;
    Assert::AreEqual(
        m7, MatrixFromRows(Vec4f{2.0f, 4.0f, 6.0f, 8.0f}, Vec4f{10.0f, 12.0f, 14.0f, 16.0f},
    ...
    Assert::AreEqual(scale, Mat4f{Vec4f{3.0f, 0.0f, 0.0f, 0.0f}, Vec4f{0.0f, 3.0f, 0.0f, 0.0f},
    ...
    Assert::IsTrue(std::equal(std::cbegin(m4), std::cend(m4),
                              stdext::make_unchecked_array_iterator(
                                  std::begin({1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f,
...
TEST_METHOD(TestLookAtRhMat4f) {
    ...
    const auto xmm0 = DirectX::XMMatrixLookAtRH(toXmVector(Vec4f{eyePos, 0}),
                                                toXmVector(Vec4f{at, 0}), toXmVector(Vec4f{up, 0}));
    Assert::IsTrue(areNearlyEqual(m0, toMat4f(xmm0), 1e-6f));
}
